program bandstructure
  ...
  do q=1, nqpoints_mode_susc
    ...
    call compute_freq_x_mom4(mode_susc, mom4_inp, qp)
...
subroutine compute_freq_fc(qp, eig, eigvec)
...
subroutine compute_freq_fc_mom4(mom4_inp, qp)  
  real(kind=wp), intent(in):: mom_inp(:,:)  
  real(kind=wp), intent(in):: qp(3)
  
  real(kind=wp):: mom2, mom4, mom6, d1,d2,d3, mom_out(3), fc_q_n
  ...
  call compute_mom4_new2(eigvec, mom4_inp(:,:), mom4_out)
  ...
  do i=1,3
    ...
    call estimate_parameters_gaussian(eig(i), mom4_out(i))
...
!subroutine compute_freq_x(qp, eig, eigvec)
!  real(kind=wp), intent(in):: qp(3)
!  real(kind=wp), intent(out):: eig(:), eigvec(:,:)
!
!  call diagonalize(mode_susc(:,:,q), eig, eigvec)
!
!end subroutine compute_freq_x

subroutine compute_freq_x_mom4(mode_susc, mom4_inp, qp)
  ...
  call compute_mom4_new2(eigvec, mom4_inp(:,:,q), mom4_out)
  
  do i=1,3
     mode_susc_n = dot_product(eigvec(:,i), matmul(mode_susc(:,:,q), eigvec(:,i) ) ) 
...
subroutine compute_mom4_new2(eigvec, mom4_inp, mom4_out)
  real(kind=wp), intent(in):: eigvec(3,3)
  real(kind=wp), intent(out):: mom4_inp(3,3)
...
subroutine compute_fc_q_real(supercell, Mat_symm, qp, fc_q_real)
  integer, intent(in):: supercell(3)
  integer, intent(in):: Mat_symm(:,:,:)
  real(kind=wp), intent(in):: qp(3)
